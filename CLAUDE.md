...
- **Endpoints** (via `google.script.run`) : signatures dans `Alfred.js`. `getAllData()` = tout en un appel → `{forecast, rules, options, savingsProps, shownAccounts(+balance), tasks, prevs}`. Soldes EB en parallèle via `_getAccountBalances` (`fetchAll`).
