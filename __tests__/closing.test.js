...
describe('_closingBalances_ — frontière de confiance (Phase 3)', () => {
  test('soldes client fournis (numériques) → utilisés tels quels', () => {
    expect(g._closingBalances_({ lep: 9800.5, la: 5000, csl: 1000 }))
  ...
  test('soldes absents → erreur (pas d\'archive de données obsolètes)', () => {
    ...
    expect(() => g._closingBalances_(null)).toThrow(/manquants/);
    expect(() => g._closingBalances_(undefined)).toThrow(/manquants/);
  });

  test('payload invalide (lep non numérique) → erreur, pas de confiance aveugle', () => {
    expect(() => g._closingBalances_({ lep: '9800', la: 5000, csl: 1000 })).toThrow(/manquants/);
  });
});

describe('_collectClosingInfo_ — ne lit plus les soldes épargne', () => {
  test('retourne prevsTotal mais plus lepBal/laBal/cslBal', () => {
    const r = g._collectClosingInfo_(2026, 5);
